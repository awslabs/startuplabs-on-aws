-- Fixed version with correct column names for newer PostgreSQL versions
with get_sql_id as (
...
round(avg(temp_blks_written)) AS temp_blks_written, round(avg(shared_blk_read_time)) AS blk_read_time, round(avg(shared_blk_write_time)) AS blk_write_time
...
and query not in ('ROLLBACK','COMMIT','BEGIN')
