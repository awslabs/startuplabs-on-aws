...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Grant each runtime execution role permission to query the Bedrock Knowledge Base.
    // The default execution role only includes bedrock:InvokeModel, so RAG retrieval
    // (bedrock:Retrieve / RetrieveAndGenerate) must be added explicitly.
    const knowledgeBaseId = process.env.KNOWLEDGE_BASE_ID ?? 'REPLACE_WITH_YOUR_KB_ID';
    const knowledgeBaseRegion = process.env.KNOWLEDGE_BASE_REGION ?? 'us-east-1';
    const memoryId = process.env.AGENTCORE_MEMORY_ID ?? 'REPLACE_WITH_YOUR_MEMORY_ID';
    for (const env of this.application.environments.values()) {
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          actions: ['bedrock:Retrieve', 'bedrock:RetrieveAndGenerate'],
          resources: [
            `arn:aws:bedrock:${knowledgeBaseRegion}:${this.account}:knowledge-base/${knowledgeBaseId}`,
          ],
        })
      );

      // Grant AgentCore Memory data-plane access for persistent cross-session
      // memory (store events, retrieve/list memory records).
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          actions: [
          ...
          resources: [
            `arn:aws:bedrock-agentcore:${knowledgeBaseRegion}:${this.account}:memory/${memoryId}`,
            `arn:aws:bedrock-agentcore:${knowledgeBaseRegion}:${this.account}:memory/${memoryId}/*`,
          ],
        })
      );
